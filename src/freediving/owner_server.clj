  ...
  (with-open [c (DriverManager/getConnection url)]
    ...
    (doseq [table ["extractions" "observations" "review_proposals" "review_decisions" "publication_decisions" "publication_policy_events" "correction_requests" "correction_triage"]]
      (let [v (first (query c (str "SELECT has_table_privilege(current_user,'freediving." table "','SELECT') AS readable,has_table_privilege(current_user,'freediving." table "','UPDATE,DELETE,TRUNCATE') AS mutable,has_table_privilege(current_user,'freediving." table "','INSERT') AS appendable")))]
        (when (or (not (:readable v)) (:mutable v) (not= (and review-enabled? (contains? #{"review_proposals" "review_decisions" "publication_decisions" "correction_triage"} table)) (:appendable v))) (fail! 403 (if review-enabled? "Reviewer table privileges invalid" "Inspector table privileges invalid")))))
    (when (or (seq (query c "SELECT oid FROM pg_namespace WHERE nspname NOT LIKE 'pg_%' AND nspname <> 'information_schema' AND (pg_has_role(current_user,nspowner,'MEMBER') OR has_schema_privilege(current_user,oid,'CREATE'))"))
              ...
              (seq (query c "SELECT p.oid FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace WHERE p.prosecdef AND n.nspname NOT LIKE 'pg_%' AND n.nspname <> 'information_schema' AND has_function_privilege(current_user,p.oid,'EXECUTE')"))
              (seq (query c "SELECT c.oid FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname NOT LIKE 'pg_%' AND n.nspname <> 'information_schema' AND c.relkind IN ('r','p','v','m','f') AND (pg_has_role(current_user,c.relowner,'MEMBER') OR has_table_privilege(current_user,c.oid,'TRUNCATE,REFERENCES,TRIGGER') OR has_any_column_privilege(current_user,c.oid,'REFERENCES') OR (NOT (n.nspname='freediving' AND c.relname='public_projection_cache') AND (has_any_column_privilege(current_user,c.oid,'UPDATE') OR has_table_privilege(current_user,c.oid,'DELETE'))) OR (NOT (n.nspname='freediving' AND c.relname IN ('review_proposals','review_decisions','publication_decisions','correction_triage','evaluation_labels','public_projection_cache')) AND has_any_column_privilege(current_user,c.oid,'INSERT')))")))
            ...
            "/api/publication" (do
                                 (when (and source-config (= :validate (:action r)))
                                   (let [t (select-keys r [:job-id :ordinal])
                                     ...
                                     (when-not (and row seen (= page (:page seen)))
                                       (fail! 403 "View this observation's source page before attesting accuracy"))
                                     (let [current (try (:metadata ((requiring-resolve 'freediving.source-pages/render!) source-config row page))
