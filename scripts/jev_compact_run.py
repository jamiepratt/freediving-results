#!/usr/bin/env python3
"""One-shot compact question-local Jev comparison launcher. A durable start is irrevocable, even on failure."""
import argparse
import json
import os
...
def validate(path):
    ...
    required = {Path(sys.executable).resolve(), Path('/opt/homebrew/opt/openjdk@21/libexec/openjdk.jdk/Contents/Home/bin/java').resolve(), Path(__file__).resolve(), cwd / 'scripts/jev_compact_runner.clj', cwd / 'scripts/jev_frozen_runner.clj', cwd / 'scripts/jev_frozen_run.py', cwd / 'deps.edn'}
    ...
    if type(m.get('max_http')) is not int or not 11 <= m['max_http'] <= 81:
        raise RuntimeError('Invalid dispatch bound')
    rows = json.loads((packet / 'requests.json').read_text())
    if len(rows) != m['max_http'] or sum(len(row['case-ids']) for row in rows) != 81:
...
def run_clojure(m, mode, key=None):
    ...
    command = ['clojure', '-Sdeps', '{:paths ["src" "resources" "scripts"]}', '-M', '-m', 'jev-compact-runner', mode, m['packet'], m['root'], m['original_packet'], m['expected_run_id']]
