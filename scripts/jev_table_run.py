#!/usr/bin/env python3
"""One-shot table question-local Jev comparison launcher. A durable start is irrevocable, even on failure."""
import argparse
import json
import os
...
def validate(path):
    ...
    required = {Path(sys.executable).resolve(), Path('/opt/homebrew/opt/openjdk@21/libexec/openjdk.jdk/Contents/Home/bin/java').resolve(), Path(__file__).resolve(), cwd / 'scripts/jev_table_runner.clj', cwd / 'scripts/jev_frozen_runner.clj', cwd / 'scripts/jev_frozen_run.py', cwd / 'deps.edn'}
    ...
    if type(m.get('max_http')) is not int or m['max_http'] != 17:
