...
SPEC = importlib.util.spec_from_file_location('runner', Path(__file__).with_name('jev_larger_run.py'))
...
class DurableLaunch(unittest.TestCase):
    def test_missing_credential_can_launch_after_correction(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            manifest = {'root': str(root), 'cwd': str(root), 'packet': str(root), 'original_packet': str(root), 'expected_run_id': '0' * 64}
...
class FrozenPacketCheck(unittest.TestCase):
    @unittest.skipUnless(os.environ.get('JEV_LARGER_TEST_PACKET'), 'private frozen packet unavailable')
    def test_complete_clojure_entrypoint_checks_without_creating_store(self):
        runner = importlib.util.module_from_spec(SPEC)
        SPEC.loader.exec_module(runner)
        with tempfile.TemporaryDirectory() as tmp:
            m = {'root': tmp, 'cwd': str(Path(__file__).resolve().parent.parent), 'packet': os.environ['JEV_LARGER_TEST_PACKET'], 'original_packet': os.environ['JEV_ORIGINAL_TEST_PACKET'], 'expected_run_id': os.environ['JEV_LARGER_TEST_RUN_ID']}
    ...
    def test_full_frozen_live_replay_and_terminal_arm_stop_with_fake_provider(self):
        for terminal, expected_calls in ((False, 17), (True, 1)):
            with self.subTest(terminal=terminal), tempfile.TemporaryDirectory() as tmp:
                ...
                packet = os.environ['JEV_LARGER_TEST_PACKET']
                form = r'''(require '[jev-larger-runner :as r] '[jev-frozen-runner :as original] '[freediving.evaluation-providers :as p] '[clojure.java.io :as io])
(def calls (atom 0))
...
(assert (= EXPECTED @calls))
'''.replace('TERMINAL', str(terminal).lower()).replace('EXPECTED', str(expected_calls)).replace('PACKET', json.dumps(packet)).replace('ROOT', json.dumps(str(root))).replace('ORIGINAL', json.dumps(os.environ['JEV_ORIGINAL_TEST_PACKET'])).replace('RUNID', json.dumps(os.environ['JEV_LARGER_TEST_RUN_ID']))
                ...
                self.assertEqual({'L5'}, set(analysis['providers']))
...
"""
            ...
            packet = Path(os.environ['JEV_LARGER_TEST_PACKET'])
            files = [cwd / 'scripts/jev_larger_run.py', cwd / 'scripts/jev_larger_runner.clj', cwd / 'scripts/jev_frozen_runner.clj', cwd / 'scripts/jev_frozen_run.py', cwd / 'deps.edn', Path(sys.executable).resolve(), Path('/opt/homebrew/opt/openjdk@21/libexec/openjdk.jdk/Contents/Home/bin/java').resolve()]
            ...
            manifest.write_text(json.dumps({'root': str(root), 'cwd': str(cwd), 'packet': str(packet), 'original_packet': str(original), 'expected_run_id': os.environ['JEV_LARGER_TEST_RUN_ID'], 'files': {str(p): hashlib.sha256(p.read_bytes()).hexdigest() for p in files}}))
            def cli(mode):
                return subprocess.run([sys.executable, str(cwd / 'scripts/jev_larger_run.py'), mode, '--manifest', str(manifest)], env=env, capture_output=True, text=True)
            first = cli('live')
