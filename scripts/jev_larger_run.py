#!/usr/bin/env python3
"""One-shot five-question Jev comparison launcher. A durable start is irrevocable, even on failure."""
import argparse
import json
import os
...
def validate(path):
    ...
    required = {Path(sys.executable).resolve(), Path('/opt/homebrew/opt/openjdk@21/libexec/openjdk.jdk/Contents/Home/bin/java').resolve(), Path(__file__).resolve(), cwd / 'scripts/jev_larger_runner.clj', cwd / 'scripts/jev_frozen_runner.clj', cwd / 'scripts/jev_frozen_run.py', cwd / 'deps.edn'}
...
def run_clojure(m, mode, key=None):
    ...
    command = ['clojure', '-Sdeps', '{:paths ["src" "resources" "scripts"]}', '-M', '-m', 'jev-larger-runner', mode, m['packet'], m['root'], m['original_packet'], m['expected_run_id']]
...
def main(argv=None):
    ...
    if args.mode == 'live':
        ...
        save(start, {'run_id': m['expected_run_id'], 'manifest_sha256': sha(args.manifest), 'max_http': 17, 'max_questions': 81, 'attempts': 1, 'concurrency': 1, 'started_at': time.time()})
        try:
    ...
    print(json.dumps({'mode': args.mode, 'run_id': m['expected_run_id'], 'status': 'ok', 'prepared_objects_verified': 17, 'wire_hashes_verified': 17, 'original_questions_verified': 81}))
    return 0
