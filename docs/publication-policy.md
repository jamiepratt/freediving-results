...
The development helper uses loopback trust authentication. Another local process can impersonate a role. These tests verify privileges within a database session, not production authentication. No HTTP service, public login boundary or deployment is provided here. Protect private reports and reviewer credentials. Reviewer-approved public text must itself be suitable for public display; an allowlist of fields is not a secret scanner for intentionally supplied strings.
