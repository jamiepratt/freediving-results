...
The separate schema-5 CMAS JSON extraction review audit records an owner's accuracy acceptance for an original zero-based JSON row and the cited HTML Results page. It binds the source, extraction artifact, parser version, candidate, row, and owner response receipt in an append-only reviewer-role event. This narrower acceptance is not a publication validation. It supplies no substantive-error, source-scope, identity, relationship, completeness, or selection decision and does not change public eligibility. A later revocation preserves its history.
...
The current [isolated B16 corpus](championship-corpus-20260925.md) preserves all 55 B11 versions and adds two immutable indoor `/2` versions: 57 versions and 5,384 versioned observations. Diagnostics find 1,016 structurally ready rows, including 412 newly imported distance rows; these counts are not unique attempts. Every row remains unreviewed and ineligible, with zero public rows and unchanged initial policy 1. Historical `/1` indoor versions retain their blockers; all 236 timing rows in `/2` remain blocked. The 44 legacy men's rows retain evidence-envelope limitations despite automated readiness. AIDA requires separately authorized policy 2 and fresh validation. No real review authority was added.
