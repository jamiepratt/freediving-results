...
Synthetic labels require `:provenance :synthetic` and a fixture ID. Owner labels require `:provenance :owner`, review ID, reviewer, ISO timestamp `:reviewed-at`, `:review-artifact-sha256`, and reviewed evidence IDs belonging to the case. These are explicit provenance assertions; this file API does not authenticate reviewers, query the review database or verify referenced source/review bytes. A trusted export must establish those facts before a real evaluation. Rejected proposals, retrieval outcomes and matching source names are never automatically converted into labels.
...
Executable tests verify this function contract and repeated-node local persistence using an injected callback. Actual Rama registration/execution, resolver wiring and deployment are not verified here. No companion source was changed. Those are explicit integration checkpoints in issue #1; this shim is not a claim of a deployed Agent-o-rama pipeline.
