...
This slice registers existing source bytes and explicit acquisition provenance in a private local archive, then produces private page-preserving extraction artifacts and structured CMAS CWT men, AIDA Wakayama rankings and CMAS Athens DNF/DYNBF/DYN candidates. It does not implement the PostgreSQL observations database, resolve identities, or publish data. Pilot acceptance and remaining scope live in [issue #1](https://github.com/jamiepratt/freediving-results/issues/1), which remains open.
