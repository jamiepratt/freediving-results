...
This slice registers source bytes and acquisition provenance in a private archive, produces versioned PDF extraction artifacts, imports those artifacts into immutable PostgreSQL observations, and records reversible owner review decisions separately. It supports CMAS CWT men, CMAS 2025-2026 senior/master depth layouts, AIDA Wakayama rankings and CMAS Athens distance, STA and speed candidates. The 2026 Novi Sad parsers retain 412 distance rows and [236 STA/speed rows](docs/cmas-timing-acquisition-20260925.md#batch-7-2026-indoor-sta-and-speed), with printed timing ambiguities explicitly unresolved. Limited isolated owner review now exists; it grants no production publication. Current acquisition scope is tracked in [issue #8](https://github.com/jamiepratt/freediving-results/issues/8); the [25 September championship inventory](docs/championship-inventory-20260925.md) records acquired sources and unresolved coverage, including 1,942 reconciled AIDA rows and retained CMAS timing documents. Automated reconciliation supplies no reviewer attestation. Identity evaluation remains separate in [issue #6](https://github.com/jamiepratt/freediving-results/issues/6).
...
The [Novi Sad semantics audit](docs/indoor-semantics-audit-20260925.md) preserves all 648 source rows while confining timing warnings to 236 timing candidates in new parser versions. Separate diagnostics make 412 distance rows structurally ready; every row remains unreviewed and ineligible. The existing corpus is unchanged.
...
The schema-3 parser `cmas-novi-sad-dnf-juniors/1` supports the June 11, 2026 Novi Sad junior DNF layout: 11 rows on pages 1-2 of the 32-page source. It preserves all pages and raw values, explicit metre units, category punctuation and DSQ notes; blank finals and absent penalties remain unknown. Pages 3-32 remain unsupported with unknown row counts, and reconciliation counts cover supported pages only. All candidates require owner review. Pipeline job identities bind the parser inventory, so existing registrations require fresh registration; old receipts remain unchanged.

The women depth parser `cmas-women-depth/1` supports the independently inspected 2025 CWT, FIM and CNF layouts. All three sources print a declared-depth column, including FIM. It retains that column as `:declared-depth`, measured `DEPTH` as the existing `:attempted-depth`, and `FINAL DEPTH` as `:final-depth`. Printed penalties, tied ranks, unranked DNS/DSQ, exact names and representation codes remain separate. Blank status and absent units remain unknown. Standalone table dates supply event-date evidence; `Report Created` timestamps remain exact footer evidence without an inferred timezone or event date.

The additional schema-2 parser supports inspected 2025 senior men, senior women CWT-BF and masters layouts across CWT, FIM, CNF and CWT-BF. It binds rows to printed discipline/category sections, including combined masters subsections and inline categories, and handles split column headings, explicit DSQ penalties, DNS zero depth and printed record text. The text-only `cmas-2025-depth/1` path retains its historical behavior. PDF extraction now uses `cmas-2025-depth/2`, adding Poppler word positions for evidenced headerless continuations. Exact row and metadata evidence remain attached. Missing or contradictory context, malformed headers and ambiguous rows stay unparsed. No units, nationality, identity, card or successful status are inferred.

Across the 20 acquired 2025 timing PDFs, 23 pages contain 352 independently reconciled, parsed source rows. This includes the 20 previously unresolved continuation rows. Seventeen PDFs have new `/2` jobs alongside their historical jobs; the three women's PDFs retain `cmas-women-depth/1`. See [batch 4 evidence](docs/cmas-timing-acquisition-20260925.md#batch-4-geometry-backed-continuations) for reconciliation and private artifacts. All rows remain unreviewed and publication-blocked.

Continuation extraction requires matching event/date, adjacent numbered pages, report timestamp, page dimensions and unambiguous previous-page table context. Word positions establish separate numeric columns; blank interior cells never shift later values. Raw bbox XML, matched row words, header geometry and exact page/line context supplement schema 2's existing evidence. Tool arguments and parser version bind distinct job identities. Before observation import, `/2` artifacts replay layout and geometry from the hash-verified PDF and reject changed fields or geometry, including rehashed artifacts. Replay requires the recorded Poppler version. Historical schemas and artifacts remain intact; no database migration is required.

Schema-2 parser `cmas-2026-depth/1` supports the inspected Roatan depth tables using each page's printed discipline, date, columns and category headings. Geometry keeps declared, attempted and final results separate across blank cells, with wrapped notes, medals and records retained independently. Unknown or competing category boundaries, malformed context and ambiguous numeric positions fail closed. Table dates remain distinct from championship dates and report timestamps. Units, blank status, identity and citizenship remain unknown. Source replay validates the versioned geometry artifact before import. The [batch 5 evidence](docs/cmas-timing-acquisition-20260925.md#batch-5-2026-depth-reconciliation) accounts for seven retained PDFs; missing CWT-men sessions remain explicit coverage gaps.

Schema-2 parser `cmas-2026-indoor-time/1` retains all 648 rows in the two inspected Novi Sad PDFs, including the unchanged 412 distance candidates. The 236 additional STA/speed candidates preserve exact value strings, source labels and geometry. STA prints conflicting distance/metre headings; speed 2X50/4X50 leaves its first value column unlabeled. No realized-time meaning, seconds conversion or numerical penalty is inferred. Speed final-time components describe notation only, including literal zero and three-component colon tokens. Normalized durations remain unknown. Every timing page requires its own printed context. Versioned source replay and pipeline registration preserve earlier distance and junior-only jobs. The new artifacts remain blocked by unresolved source semantics as well as owner review; extraction coverage does not establish publication readiness. See [batch 7 evidence](docs/cmas-timing-acquisition-20260925.md#batch-7-2026-indoor-sta-and-speed).

The women depth artifacts reuse schema 2's multiline candidate evidence contract. An adjacent, aligned medal continuation after `EARLY TURN` is attached with exact source lines, note fragments and an explicit joining repair. Malformed rows, missing column context and tokens crossing column boundaries stay unparsed. Every nonblank line is retained as candidate evidence or classified metadata. Parser identity creates new jobs alongside historical unsupported artifacts; unchanged reruns reuse the stored artifact. Pipeline registration includes this parser version. The public field allowlist retains declared depth separately, but publication still requires the existing owner validation gates.

Independent rendering and text reconciliation found 29 CWT, 30 FIM and 18 CNF result rows, all 77 parsed, with 105 nonblank lines accounted for (including FIM's additional note line). Disposable local PostgreSQL verification imported 77 observations, preserved exact candidate/evidence payloads, and confirmed duplicate-free extraction/import reruns. These are extraction/import counts: genuine reviewed cases remain zero. Only synthetic fixtures are committed.
