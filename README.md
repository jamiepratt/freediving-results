...
The additional schema-2 parser `cmas-2025-depth/1` supports inspected 2025 senior men, senior women CWT-BF and masters layouts across CWT, FIM, CNF and CWT-BF. It binds rows to printed discipline/category sections, including combined masters subsections and inline categories, and handles split column headings, explicit DSQ penalties, DNS zero depth and printed record text. Exact row and metadata evidence remain attached. Missing or contradictory context, malformed headers and ambiguous rows stay unparsed. Headerless continuation pages remain unresolved because numeric token counts cannot establish blank column positions. No units, nationality, identity, card or successful status are inferred.

Across the 20 acquired 2025 timing PDFs, 23 pages contain 352 source rows: 332 parsed and 20 headerless continuation rows unparsed. Seventeen PDFs use the new parser; the three previously supported women's PDFs retain `cmas-women-depth/1` and their historical jobs. Earlier parser versions remain available, and HTML/2026 parsing behavior is unchanged. See [batch 3 evidence](docs/cmas-timing-acquisition-20260925.md#batch-3-2025-depth-extraction) for scope and private artifacts. All rows remain unreviewed and publication-blocked.
