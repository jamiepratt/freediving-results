...
(function () {
  ...
  function pageViewer(request, show) {
    ...
    return {
      ...
      async load(target,page){
        ...
        try {
          if(target['source-format']==='html'){const metadata=await request('/api/source-html?'+new URLSearchParams({'job-id':target['job-id'],ordinal:target.ordinal}));if(current===ticket)show({state:'html',target,page:1,metadata});return;}
  ...
  function common(f){return {...f,actor:$('actor').value,...(detail?.packet.target['source-format']==='html'?{table:$('page').value,row:$('line').value}:{page:$('page').value,line:$('line').value})};}
  ...
  const pageView=pageViewer(api, view=>{
    ...
    $('source-navigation').hidden=view.target?.['source-format']==='html';
    $('source-title').textContent=view.target?.['source-format']==='html'?'Registered HTML row and context':'Registered PDF page';
